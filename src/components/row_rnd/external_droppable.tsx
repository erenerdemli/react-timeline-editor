...
export const RowDnd = React.forwardRef<RowRndApi, RowRndProps>(
  (
    ...
    const move = (param: { preLeft: number; preWidth: number; preTop: number; scrollDelta?: number }) => {
      const { preLeft, preWidth, preTop, scrollDelta } = param;
    ...
    const handleMoveStop = (e: DragEvent) => {
      ...
      const target = e.target;
      let { left, width, top } = target.dataset;
      onDragEnd && onDragEnd({ left: parseFloat(left), width: parseFloat(width), top: parseFloat(top) });
    };
